...
def parse_opt(known=False):
    ...
    parser.add_argument('--imagesize', type=int, default=640, help='train, val image size (pixels)')
    ...
    for yolos_name in yolos_names:
        if yolos_name.lower().endswith(('.py')):
            ...
            if total_step <= wanted_step:
                ...
                # print("\033[1;33;44m[Warning] 请在训练前修改UnFreeze_Epoch为%d，或者修改Unfreeze_batch_size为%d。\033[0m
            if True:
            ...
            num_neck += 1
    shutil.copy('static/evalu_parames.txt', 'train_results/%s_%s_%s.txt'%(backbone, UnFreeze_Epoch, Unfreeze_batch_size))
    shutil.copy('static/train.txt', 'train_results/train_%s_%s_%s.txt'%(backbone, UnFreeze_Epoch, Unfreeze_batch_size))
