...
'''
LossHistory : トレーニング中の損失と検証損失を記録し、損失カーブをプロットするためのもの

ExponentDecayScheduler : 各エポックの終了時に指定された減衰率に従って学習率を更新するためのもの

WarmUpCosineDecayScheduler : 学習の開始時に学習率のウォームアップを行い、各エポックの終了時に、コサイン退火ストラテジーに従って学習率を更新するためのもの

EvalCallback : 各エポックの終了時にモデルのパフォーマンスを評価し、mAPを計算するためのもの
'''
...
class EvalCallback(keras.callbacks.Callback):
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        #---------------------------------------------------------#
        #   ここで画像をRGB画像に変換し、グレースケール画像が予測時にエラーを出さないようにします
        #---------------------------------------------------------#
        image = cvtColor(image)
        #---------------------------------------------------------#
        #   画像にグレーバーを追加し、歪みのないリサイズを実現します. また、直接リサイズして認識することもできます
        #---------------------------------------------------------#
