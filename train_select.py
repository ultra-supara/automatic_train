...
def parse_opt(known=False):
    ...
    parser.add_argument('--neck', type=str, default='', help='neck')
    parser.add_argument('--batch-size', type=int, default='', help='total batch size for all GPUs, -1 for autobatch')
    parser.add_argument('--imagesize', type=int, default='', help='train, val image size (pixels)')
...
if __name__ == "__main__":
    ...
    model_path      = ''
    input_shape     = [opt.imagesize, opt.imagesize]
    mosaic              = True
    mosaic_prob         = 0.5
    mixup               = True
    mixup_prob          = 0.5
    ...
    if os.path.exists('modelsummary.txt'):
        os.remove('modelsummary.txt')
    print('Training model: {} +'.format(neck),'{}!'.format(backbone))

    if ngpus_per_node > 1:
        with strategy.scope():
            if neck == 'yolox':
                model_body  = yolo_body((opt.imagesize, opt.imagesize, 3), num_classes, backbone)
                if model_path != '':
                ...
                model_body  = yolo_body((opt.imagesize, opt.imagesize, 3), anchors_mask, num_classes, backbone)
                if model_path != '':
                    print('Load weights {}.'.format(model_path))
                    model_body.load_weights(model_path, by_name=True, skip_mismatch=True)
                if not eager:
    ...
    if True:
        ...
        if epoch_step == 0 or epoch_step_val == 0:
            raise ValueError('データセットが小さすぎて、学習を行うことができません。データセットを拡充してください。')
        if neck =='yolov3':
    ...
    map,fps = get_mapfps(neck,backbone,opt.imagesize)
    ...
    shutil.copy('static/evalu_parames.txt', 'train_results/results_%s+%s.txt'%(neck, backbone))
    shutil.copy('static/train.txt', 'train_results/train_%s+%s.txt'%(neck, backbone))
